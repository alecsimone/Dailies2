 ...
 * as desired.
 */
var gfyCollection = function () { //Maybe change this name to be gfyReplacement
   ...
   // var collection = [];
    ...
    function init() { //My code:
        ...
        for (var i = 0; i < elem_coll.length; i++) { //And we kill this for loop
            var gfyObj = new gfyObject(elem_coll[i]); //so here we can just kill that i
            console.log(i); 
            collection.push(gfyObj); //We can also skip this step of the collection and go straight to gfyObj.init, I believe
        }
...
window.addEventListener('load',function(){
    var allGFYs = document.getElementsByClassName('gfyitem');

    for (var i=0; i<allGFYs.length; i++) {
        allGFYs[i].addEventListener("click",gfyCollection.init,false); // This is the first thing you need to attack. Change the listeners to be for clicks on items with a class, and on the click grab the ID of the clicked element. Or a data, point is just to get the GFYtitle
    };
});